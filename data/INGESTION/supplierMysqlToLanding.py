...
spark = SparkSession.builder.appName("SupplierPostgreToBronze").getOrCreate()

# Google Cloud Storage (GCS) Configuration
GCS_BUCKET = "retailer-raw-data-11111"
BRONZE_PATH = f"gs://{GCS_BUCKET}/bronze/supplier-db/"
ARCHIVE_PATH = f"gs://{GCS_BUCKET}/bronze/supplier-db/archive/"
...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://34.132.248.110:3306/supplier_db?useSSL=false&allowPublicKeyRetrieval=true",
...
def move_existing_files_to_archive(table):
    blobs = list(storage_client.bucket(GCS_BUCKET).list_blobs(prefix=f"bronze/supplier-db/{table}/"))
    ...
    for file in existing_files:
        ...
        # Move to Archive
        archive_path = f"bronze/supplier-db/archive/{table}/{year}/{month}/{day}/{file.split('/')[-1]}"
...
def extract_and_save_to_bronze(table, load_type, watermark_col):
    try:
        ...
        JSON_FILE_PATH = f"bronze/supplier-db/{table}/{table}_{today}.json"
